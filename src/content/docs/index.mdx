---
title: Architecture Guardrails for AI Coding Agents
...
<Card title="Context Package (RAG)" icon="rocket">
  <img class="demo-thumb" src="/og/live-context-pack-thumb.png" alt="Adaptive Context Package demo — ranked file list toggling between --adaptive=off and --adaptive=conservative" width="600" height="315" loading="lazy" />
...
<CardGrid>
  <Card title="Doc ↔ Code Map" icon="document">
    <img class="demo-thumb" src="/og/live-doc-code-map-thumb.png" alt="Doc ↔ Code Map demo — documentation, grounded source files, and evidence columns" width="600" height="315" loading="lazy" />
  ...
  <Card title="Rules Catalog" icon="approve-check-circle">
    <img class="demo-thumb" src="/og/live-rules-catalog-thumb.png" alt="Rules Catalog demo — structural rule card with forbidden import patterns table" width="600" height="315" loading="lazy" />
...
iw intent check --changed src/auth.ts --severity high --format text
```

* **`iw intent extract` options:** `--provider` (e.g. openai), `--output` (specify rules file path).
* **`iw intent check` options:** `--changed <files>` (comma-separated, incremental CI mode), `--severity <level>` (filter by high/medium/low), `--rule-id <id>` (check specific rule), `--format <format>` (json/text). Runs as `iw index rules-check` under the hood — see the [CLI Reference](/docs/reference/cli/) for every option.
...
─────────────────                  ──────────────────────                 ────────────────────
iw index build                     iw plugin add llm                     iw plugin add kg
